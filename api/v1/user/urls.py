...
router.register(
    "user_notification", views.UserNotificationViewSet, basename="user_notification"
)

router.register("ticket_room", views.TicketRoomViewSet, basename="ticket_room")

ticket_room_router = routers.NestedDefaultRouter(
    router, "ticket_room", lookup="ticket_room"
)
ticket_room_router.register(
    "ticket_chat", views.TicketChatViewSet, basename="ticket_chat"
)

ticket_chat_router = routers.NestedDefaultRouter(
    ticket_room_router, "ticket_chat", lookup="ticket_chat"
)
...
urlpatterns = [
    ...
    path(
        "otp_phone_verify/",
        views.RequestOtpVerifyView.as_view(),
        name="otp_phone_verify",
    ),
    ...
    path(
        "state-list/<int:pk>/", views.StateApiView.as_view(), name="detail-state-list"
    ),
    path(
        "state/<int:pk>/city/",
        views.StateCitiesGenericView.as_view(),
        name="detail-state-city",
    ),
