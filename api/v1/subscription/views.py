...
class SubscriptionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        query = Subscription.objects.select_related(
            "user", "course__category", "coupon", "crud_course_type"
        ).only(
            "course__course_name",
...
class PaymentSubscriptionViewSet(
    ...
    def get_queryset(self):
        return (
            PaymentSubscription.objects.filter(subscription__user=self.request.user)
            .only(
                "subscription__user",
                "subscription__price",
...
class VerifyPaymentView(views.APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        params_status = request.query_params.get("status")

        if not track_id and params_status:
            raise exceptions.ValidationError(
                {"message": "please enter query params success and track_id and status"}
            )
        ...
        payment_subscription = PaymentSubscription.objects.filter(
            response_payment__trackId=int(track_id)
        ).only("response_payment")
