...
class CategoryBlogViewSet(viewsets.ModelViewSet):
    ...
    queryset = CategoryBlog.objects.filter(is_publish=True).defer(
...
class PostBlogViewSet(viewsets.ModelViewSet):
    ...
    def like(self, request, pk=None, category_pk=None):
        ...
        if not like.exists():
            ...
            raise exceptions.ValidationError(
                {"message": _("you have already like this post")}
            )

        # save like in post and get post
        PostBlog.objects.filter(is_publish=True, id=pk).only("post_title").update(
            likes=F("likes") + 1
        )
    ...
    def increment_read_count(self, request, pk=None, category_pk=None, post_slug=None):
        # update query
        PostBlog.objects.filter(is_publish=True, post_slug=post_slug).only(
            "post_title"
        ).update(read_count=F("read_count") + 1)
        return Response(
            {"status": "read count incremented"}, status=status.HTTP_201_CREATED
        )
    ...
    def get_queryset(self):
        queryset = (
            ...
            .prefetch_related(
                ...
                Prefetch("tags", queryset=TagBlog.objects.only("tag_name")),
            )
            .only(
                "author__first_name",
                "author__last_name",
    ...
    def get_permissions(self):
        if (
            ...
            or self.action == "increment_read_count"
        ):
            self.permission_classes = (permissions.AllowAny,)
...
class FavouritePostViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return FavouritePost.objects.filter(user=self.request.user).defer(
            "is_deleted", "deleted_at"
        )
...
class LatestPostViewSet(
    ...
    def get_queryset(self):
        return (
            PostBlog.objects.filter(is_publish=True)
            .only(
                "category_id",
                "post_title",
                "post_introduction",
...
class AllPostBlogViewSet(
    ...
    def get_queryset(self):
        ...
        if self.action == "retrieve":
            return base_queryset.select_related("category").prefetch_related(
                Prefetch(
                    "author", queryset=User.objects.only("first_name", "last_name")
                ),
                Prefetch("tags", queryset=TagBlog.objects.only("tag_name")),
...
class SeoPostBlogViewSet(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        query = PostBlog.objects.filter(is_publish=True).only(
            "post_title", "post_slug", "created_at", "updated_at"
        )
