...
class PostBlog(CreateMixin, UpdateMixin, SoftDeleteMixin):
    author = models.ManyToManyField(User, related_name="post_authors")
    category = models.ForeignKey(
        CategoryBlog, on_delete=models.PROTECT, related_name="blog_posts"
    )
    post_introduction = models.CharField(
        max_length=255, help_text=_("مقدمه ای در مورد پست")
    )
    ...
    read_count = models.PositiveIntegerField(
        default=0, help_text=_("چند نفر این پست را دیده اند")
    )
    read_time = models.PositiveSmallIntegerField(
        help_text=_("مدت زمان برای مطالعه این مقاله")
    )
...
class FavouritePost(CreateMixin, UpdateMixin, SoftDeleteMixin):
    user = models.ForeignKey(
        User, on_delete=models.DO_NOTHING, related_name="favourite_user_posts"
    )
    post = models.ForeignKey(
        PostBlog, on_delete=models.DO_NOTHING, related_name="favourite_posts"
    )
...
class CommentBlog(CreateMixin, UpdateMixin, SoftDeleteMixin):
    user = models.ForeignKey(
        User, on_delete=models.DO_NOTHING, related_name="comment_user_posts"
    )
    post = models.ForeignKey(
        PostBlog, on_delete=models.DO_NOTHING, related_name="comment_posts"
    )
...
class Like(CreateMixin):
    user = models.ForeignKey(
        User, on_delete=models.DO_NOTHING, related_name="user_likes"
    )
    post = models.ForeignKey(
        "PostBlog", on_delete=models.DO_NOTHING, related_name="post_likes"
    )
