...
from apps.account_app.validators import (
    MobileRegexValidator,
    NationCodeRegexValidator,
    validate_upload_image_user,
)
from apps.core_app.models import UpdateMixin, SoftDeleteMixin, CreateMixin, State, City


# TODO, move in file choices
...
class User(
    AbstractBaseUser, PermissionsMixin, UpdateMixin, SoftDeleteMixin, CreateMixin
):
    ...
    second_mobile_phone = models.CharField(
        _("شماره تماس دوم"),
        max_length=11,
        blank=True,
        null=True,
    ...
    gender = models.CharField(
        _("gender"), max_length=6, choices=Gender.choices, blank=True, null=True
    )

    grade = models.CharField(
        _("grade"), max_length=8, choices=Grade.choices, blank=True, null=True
    )
...
class TicketRoom(CreateMixin, UpdateMixin, SoftDeleteMixin):
    ...
    user = models.ForeignKey(
        "User", on_delete=models.DO_NOTHING, related_name="ticker_room"
    )
...
class Ticket(MP_Node, CreateMixin, UpdateMixin, SoftDeleteMixin):
    ...
    room = models.ForeignKey(
        TicketRoom, on_delete=models.DO_NOTHING, related_name="room"
    )
...
class PrivateNotification(CreateMixin, UpdateMixin, SoftDeleteMixin):
    ...
    char_link = models.CharField(
        blank=True, max_length=100, null=True, help_text="link for redirect"
    )
