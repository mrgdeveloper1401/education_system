...
class Category(MP_Node, CreateMixin, UpdateMixin, SoftDeleteMixin):
    ...
    image = models.ImageField(
        upload_to="category_images/%Y/%m/%d", null=True, blank=True
    )
...
class Course(CreateMixin, UpdateMixin, SoftDeleteMixin):
    ...
    class Meta:
        db_table = "course"
        ordering = ("id",)  # TODO, remove this and set in api
        constraints = (
...
class CourseTypeModel(CreateMixin, UpdateMixin, SoftDeleteMixin):
    course = models.ForeignKey(
        Course, on_delete=models.CASCADE, related_name="course_type_model"
    )
...
class LessonCourse(CreateMixin, UpdateMixin, SoftDeleteMixin):
    course = models.ForeignKey(
        Course, on_delete=models.CASCADE, related_name="lesson_course"
    )
    class_name = models.CharField(help_text=_("نام کلاس"))
    coach = models.ForeignKey(
        Coach, on_delete=models.CASCADE, related_name="coach_less_course"
    )
    students = models.ManyToManyField(
        Student, related_name="student_lesson_course", through="StudentEnrollment"
    )
    is_active = models.BooleanField(
        default=True, help_text=_("دیتا در سطح اپلیکیشن نمایش داده شود یا خیر")
    )
...
class StudentEnrollment(CreateMixin, UpdateMixin, SoftDeleteMixin):
    ...
    lesson_course = models.ForeignKey(
        LessonCourse, on_delete=models.CASCADE, related_name="lesson_course_enrollment"
    )
...
class SectionFile(CreateMixin, UpdateMixin, SoftDeleteMixin):
    ...
    section = models.ForeignKey(
        Section, on_delete=models.CASCADE, related_name="section_files"
    )
...
class PresentAbsent(CreateMixin, UpdateMixin):
    ...
    student_status = models.CharField(
        choices=StudentStatusChoices.choices, default=StudentStatusChoices.nothing
    )
...
class StudentSectionScore(CreateMixin, UpdateMixin, SoftDeleteMixin):
    section = models.ForeignKey(
        Section, on_delete=models.CASCADE, related_name="section_score"
    )
...
class SendSectionFile(CreateMixin, UpdateMixin, SoftDeleteMixin):
    student = models.ForeignKey(
        Student, on_delete=models.CASCADE, related_name="send_section_files"
    )
    section_file = models.ForeignKey(
        SectionFile, on_delete=models.CASCADE, related_name="section_files"
    )
    ...
    zip_file = models.FileField(
        help_text=_("فایل ارسالی"), upload_to=student_send_section_file
    )
    comment_student = models.TextField(
        help_text=_("توضیحی در مورد تمرین ارسالی"), null=True
    )
    comment_teacher = models.CharField(
        max_length=255,
        help_text=_("توضیح مربی در مورد فایل ارسال شده دانشجو"),
        null=True,
        blank=True,
    )
...
class Certificate(CreateMixin, UpdateMixin, SoftDeleteMixin):
    section = models.ForeignKey(
        Section, on_delete=models.CASCADE, related_name="section_certificates"
    )
    student = models.ForeignKey(
        Student, on_delete=models.PROTECT, related_name="student_certificates"
    )
    unique_code = models.CharField(max_length=36, unique=True, blank=True, null=True)
    qr_code = models.ImageField(upload_to="certificates/qr_codes/%Y/%m/%d/", blank=True)
    final_pdf = models.FileField(
        upload_to="certificates/pdf/%Y/%m/%d/", null=True, blank=True
    )
    is_active = models.BooleanField(default=True)
...
class Comment(MP_Node, CreateMixin, UpdateMixin, SoftDeleteMixin):
    ...
    category = models.ForeignKey(
        Category, on_delete=models.CASCADE, related_name="category_comments"
    )
...
class OnlineLink(CreateMixin, UpdateMixin, SoftDeleteMixin):
    class_room = models.ForeignKey(
        LessonCourse, on_delete=models.CASCADE, related_name="online_link"
    )
...
class AnswerQuestion(CreateMixin, UpdateMixin, SoftDeleteMixin):
    student = models.ForeignKey(
        Student, on_delete=models.CASCADE, related_name="student_poll_answer"
    )
    section_question = models.ForeignKey(
        SectionQuestion, on_delete=models.CASCADE, related_name="section_question"
    )
...
class CallLessonCourse(CreateMixin, UpdateMixin, SoftDeleteMixin):
    lesson_course = models.ForeignKey(
        LessonCourse, on_delete=models.CASCADE, related_name="call"
    )
    call = models.CharField(max_length=50, help_text=_("تماس"))
    status = models.CharField(
        max_length=13, help_text=_("وضعیت تماس"), choices=CallStatusChoices.choices
    )
    ...
    student = models.ForeignKey(
        Student, on_delete=models.CASCADE, related_name="student_call", null=True
    )
...
class SignupCourse(CreateMixin, UpdateMixin, SoftDeleteMixin):
    course = models.ForeignKey(
        Course, on_delete=models.CASCADE, related_name="course_signup"
    )
    student_name = models.CharField(
        max_length=120, help_text="نام و نام خوادگی داشن اموز"
    )
