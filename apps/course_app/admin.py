...
class StudentEnrollmentInlineAdmin(
    admin.TabularInline,
):
...
class StudentAccessSectionAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        qs = qs.select_related("student__user", "section__course__category").only(
            "student__student_number",
            "section__title",
            "is_access",
            "created_at",
...
class SignupCourseAdmin(admin.ModelAdmin):
    ...
    def get_queryset(self, request):
        return (
            ...
            .only("course__course_name", "student_name", "phone_number", "created_at")
        )
...
class CourseTypeModelAdmin(admin.ModelAdmin):
    ...
    search_fields = ("=course__course_name",)
    search_help_text = _(
        "برای سرچ کردن میتواند از نام دوره استفاده کنید",
    )
